...
) : IAnalyticsExportService
{
    ...
    {
        var dist =
            (await analyticsService.GetStateDistributionAsync(userId, userRoles, filter)).Data
            ?? new StateDistributionDto();
        var trend =
            (await analyticsService.GetComplianceTrendAsync(userId, userRoles, filter)).Data ?? [];
        var entities =
            (await analyticsService.GetComplianceByEntityAsync(userId, userRoles, filter)).Data
            ?? [];
        var responsible =
            (await analyticsService.GetComplianceByResponsibleAsync(userId, userRoles, filter)).Data
            ?? [];
        var branches =
    ...
    private static string BuildDateRangeLabel(AnalyticsFilterRequest filter) =>
        ...
        : filter.StartDate.HasValue ? $"Desde {filter.StartDate:dd/MM/yyyy}"
        : filter.EndDate.HasValue ? $"Hasta {filter.EndDate:dd/MM/yyyy}"
        : "Reporte General";
