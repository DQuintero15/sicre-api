...
public class MonthlyReportPdfGenerator
{
    ...
    private static void Header(IContainer c, MonthlyReportData data)
    {
        c.Background(P.Brand)
         ...
         .Row(row =>
         {
             ...
             {
                 try { leftLogo.MaxHeight(40).Image(data.LogoLlanogas); }
             ...
             {
                 try { rightLogo.MaxHeight(36).Image(data.LogoCusianagas); }
                 catch { /* logo unavailable */ }
             }
         });
    }
    ...
    private static void Content(IContainer c, MonthlyReportData data)
    {
        c.Column(col =>
        {
            ...
            col.Item().PaddingTop(28)
               ...
               .Text("Este informe fue generado automáticamente por SICRE · " +
    ...
    private static void KpiSummary(IContainer c, StateDistributionDto dist)
    {
        c.Column(col =>
        {
            ...
            if (dist.Total > 0)
            {
                col.Item().PaddingTop(10).Height(7).Row(r =>
                {
                    if (dist.OnTime > 0)  r.RelativeItem(dist.OnTime).Background(P.Green);
                    if (dist.Late > 0)    r.RelativeItem(dist.Late).Background(P.Orange);
                    if (dist.Overdue > 0) r.RelativeItem(dist.Overdue).Background(P.Red);
                    if (dist.Pending > 0) r.RelativeItem(dist.Pending).Background(P.Yellow);
                });
                ...
                col.Item().PaddingTop(4).Row(r =>
                {
                    void Legend(string color, string label) =>
                        ...
                        {
                            inner.ConstantItem(8).AlignMiddle().Height(8).Width(8).Background(color);
                            inner.AutoItem().PaddingLeft(4)
    ...
    private static void KpiCard(
        RowDescriptor row, string label, int value, string color, string bg, int total)
    {
    ...
    private static void TrendSection(IContainer c, List<ComplianceTrendDto> trend)
    {
        c.Column(col =>
        {
            ...
            col.Item().PaddingTop(10).Table(table =>
            {
                ...
                table.Header(h => TableHeader(h,
                    "Mes", "Total", "A Tiempo", "Tarde", "No Report.", "Pendiente", "Cumplimiento"));
                ...
                foreach (var t in trend)
                {
                    ...
                    DataCell(table.Cell(), bg, t.OnTime.ToString(),  center: true, color: P.GreenText);
                    DataCell(table.Cell(), bg, t.Late.ToString(),    center: true, color: t.Late    > 0 ? P.OrangeText : null);
                    DataCell(table.Cell(), bg, t.Overdue.ToString(), center: true, color: t.Overdue > 0 ? P.RedText    : null);
                    DataCell(table.Cell(), bg, t.Pending.ToString(), center: true, color: t.Pending > 0 ? P.YellowText : null);
                    DataCell(table.Cell(), bg, $"{t.OnTimePercentage:F1}%", center: true, color: rateText, bold: true);
                }
            });
        });
    }
    ...
    private static void EntitySection(IContainer c, List<EntityComplianceDto> items)
    {
        c.Column(col =>
        {
            ...
            col.Item().PaddingTop(10).Table(table =>
            {
                ...
                table.Header(h => TableHeader(h,
                    "#", "Entidad de Control", "Total", "A Tiempo", "Tarde", "No Report.", "Pendiente", "Cumplimiento"));
                ...
                foreach (var item in items.OrderByDescending(x => x.OnTimeRate))
                {
                    ...
                    DataCell(table.Cell(), bg, item.OnTime.ToString(),  center: true, color: P.GreenText);
                    DataCell(table.Cell(), bg, item.Late.ToString(),    center: true, color: item.Late    > 0 ? P.OrangeText : null);
                    DataCell(table.Cell(), bg, item.Overdue.ToString(), center: true, color: item.Overdue > 0 ? P.RedText    : null);
                    DataCell(table.Cell(), bg, item.Pending.ToString(), center: true, color: item.Pending > 0 ? P.YellowText : null);
                    DataCell(table.Cell(), bg, $"{item.OnTimeRate:F1}%", center: true, color: rateText, bold: true);
                }
            });
        });
    }
    ...
    private static void BranchSection(IContainer c, List<BranchComplianceDto> items)
    {
        c.Column(col =>
        {
            ...
            col.Item().PaddingTop(10).Table(table =>
            {
                ...
                table.Header(h => TableHeader(h,
                    "#", "Sede", "Total", "A Tiempo", "Tarde", "No Report.", "Cumplimiento"));
    ...
    private static void ResponsibleSection(IContainer c, List<ResponsibleComplianceDto> items)
    {
        ...
        c.Column(col =>
        {
            ...
            col.Item().PaddingTop(10).Table(table =>
            {
                ...
                table.Header(h => TableHeader(h,
                    "#", "Responsable", "Total", "A Tiempo", "Tarde", "No Report.", "Cumplimiento"));
    ...
    {
        c.BorderTop(1).BorderColor(P.Border).PaddingTop(8).Row(row =>
        {
            row.RelativeItem()
               .DefaultTextStyle(ts => ts.FontSize(7.5f).FontColor(P.TextMuted))
    ...
    private static Action<IContainer> SectionTitle(string title) => c =>
        c.Row(row =>
        {
            row.ConstantItem(4).Background(P.Brand);
            row.RelativeItem()
               .PaddingLeft(8).PaddingVertical(6)
        ...
        bool bold = false)
    {
        var c = cell
            ...
            .PaddingHorizontal(6).PaddingVertical(5.5f);

        if (center) c = c.AlignCenter();

        c.DefaultTextStyle(ts =>
        {
            ts = ts.FontSize(8.5f);
            if (color != null) ts = ts.FontColor(color);
            if (bold) ts = ts.SemiBold();
            return ts;
        }).Text(text);
    }
}
